...
test("Ratios test, on an array of full ints, returns sum bits as skew", () => {
  ...
  expect(objectUnderTest.findRatios(payload)).toEqual(16);
});

test("Ratios test, on an array of empty ints, returns negative sum bits as skew", () => {
  const objectUnderTest = new Ratios();
...
test("Ratios test, on an array of mixed bits, returns accurate skew", () => {
  ...
  expect(objectUnderTest.findRatios(payload)).toEqual(0);
});

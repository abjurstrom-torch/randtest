...
import { OneToOneHundred, OneToOneHundredRunLengths, RandomSample, RandomSampleMassive, RandomSampleRunLengths } from "../TargetNumbers";
...
test("gatherData, on an array of full ints, returns sum bits as only run length", () => {
  ...
  expect(objectUnderTest.findRunLengths(payload)).toEqual({ 16: 1 });
});
...
test("gatherData, on an array of chunky mixed binary, returns correct run lengths", () => {
  ...
  expect(objectUnderTest.findRunLengths(payload)).toEqual({ 2: 2, 4: 3 });
});
...
test("gatherData, on a number set of random data, returns correct run lengths", () => {
  ...
  expect(objectUnderTest.findRunLengths(payload)).toEqual(expected);
});

test("testData, on a number set of random data, returns correct ratios", () => {
  const objectUnderTest = new Runs();
  const expected = {
     "1": 1,
  ...
  const testTarget = objectUnderTest.lengthsToRatios(RandomSampleRunLengths);
...
test("testData, on a large set of random data, returns a score that closely fits with expected", () => {
  ...
  const runLengths = objectUnderTest.findRunLengths(payload);
  const runScores = objectUnderTest.lengthsToRatios(runLengths);
